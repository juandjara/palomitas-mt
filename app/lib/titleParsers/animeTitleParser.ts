export default function animeTitleParser(title: string) {
  ...
  const secondSeasonQuality = /\[.*\].(.+)\W+(Se?a?s?o?n?\s?\d+|\d+\w{2}\sseason|Part\s\d+)\s\W\s(\d{2,})\D+(\d{3,4}p)/i // [HorribleSubs] Fairy Tail S2 - 70 [1080p].mkv
  const oneSeasonQuality = /\[.*\].(.+)\s\W\s(\d{2,})\D+(\d{3,4}p)/i // [HorribleSubs] Gangsta - 06 [480p].mkv
  const secondSeason = /\[.*\].(.+).(Se?a?s?o?n?\s?\d+|\d+\w{2}\sseason)\s\W\s(\d{2,}).*\.mkv/i // [Commie] The World God Only Knows S2 - 12 [C0A4301E].mkv
  const oneSeason = /\[.*\].(.+)\s\W\s(\d{2,}).*\.mkv/i // [Commie] Battery - 05 [38EC4270].mkv
...
function processTitleParts({ regex, show, season = 'S1', episode, quality = '480p' }: Record<string, string>) {
  ...
  const seasonParsed = Number(season.match(/\d+/)?.[0])

  return {
    regex,
    slug,
    show,
    key: `${slug}-S${seasonParsed}E${episode}`,
    season: seasonParsed,
